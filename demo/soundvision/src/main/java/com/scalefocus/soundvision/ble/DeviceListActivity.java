   ...
   // private BluetoothAdapter mBtAdapter;
    ...
    private ScanCallback mScanCallback = new ScanCallback()
    {
        ...
        public void onScanResult(int callbackType, ScanResult result)
        {
            ...
            final int rssi = result.getAdvertisingSid();
...
//
    ...
    private void scanLeDevice(final boolean enable) {
        ...
        if (enable) {
            ...
            mScanning = false;
            mBLEScanner.stopScan(mScanCallback);//mBluetoothAdapter.stopLeScan(mLeScanCallback);
            cancelButton.setText(R.string.scan);
        }
    ...
    private BluetoothAdapter.LeScanCallback mLeScanCallback =
        ...
        public void onLeScan(BluetoothDevice var1, int var2, byte[] var3) {
            ...
            final BluetoothDevice device = var1;
            final int rssi = var2;
            final byte[] scanRecord  = var3;
    ...
    private boolean findServiceUuidInScanRecord(byte[] scanRecord, UUID uuid){
        final char[] hexArray = "0123456789abcdef".toCharArray();
        Log.i("NLS:bt", ByteArrayToString(scanRecord));
        for(int index = 0; (index >= 0 && index < scanRecord.length-1); ) {
            ...
            //new byte[length];
            byte[] ch = Arrays.copyOfRange(scanRecord, index+1, index+1+length);
            Log.i("NLS:bt==", ByteArrayToString(ch));

            if (ch[0] == 9) {
                byte[] disp = Arrays.copyOfRange(ch, 1, length);
                try {
            ...
            if(length == 17 && header == 7){
                ...
                Log.i("NLS:bt", "returnUUID = " + stringUUID);
                if(returnUUID != null && returnUUID.equals(BLETransferService.IMAGE_TRANSFER_SERVICE_UUID))
                {
                    return true;
                }
    ...
    private void addDevice(BluetoothDevice device, int rssi, byte[] scanRecord) {
        ...
        if ( findServiceUuidInScanRecord(scanRecord, BLETransferService.IMAGE_TRANSFER_SERVICE_UUID) )
        {
